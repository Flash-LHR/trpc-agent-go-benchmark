...
- **Summary + Progressive Disclosure**：默认走 summary，在需要补充隐藏细节时允许 agent 对 hidden history 调用 `session_search` 与 `session_load`
...
- `summary_ondemand` 相对 `long_context` 的 Prompt 节省仍有 `76.69%`
- `summary_ondemand` 相对纯 `summary` 的 ROUGE-L 提升为 `+0.0255`
